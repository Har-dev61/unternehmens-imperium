...
function boot() {
    ...
    ui.init();
    document.getElementById('company-name').value = game.company.name;
    // Offline progress.
    if (savedAt) {
        ...
        if (elapsed > 60) {
          ...
          <div class="modal-actions">
            <button class="btn-prestige" data-act="ok">Super!</button>
                ...
                document.querySelector('#modal-layer [data-act="ok"]').onclick = () => ui.closeModal();
                ui.refreshAll();
            }
        }
    }
    game.start();
    // Autosave every 30 s.
    setInterval(() => { if (game.settings.autosave)
        game.save(); }, 30_000);
    // Persist on tab hide / close so progress is never lost.
    const flush = () => { if (game.settings.autosave)
        game.save(); };
