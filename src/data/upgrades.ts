...
import type { UpgradeConfig, UpgradeCategory } from '../types.js';

// --- Unlock predicate helpers ---------------------------------------------
const earned = (n: number) => (game: Game): boolean => game.player.lifetimeEarned >= n;
const clicks = (n: number) => (game: Game): boolean => game.player.totalClicks >= n;
const owns = (assetId: string, n: number) => (game: Game): boolean => (game.getAsset(assetId)?.count ?? 0) >= n;
...
function generateAssetUpgrades(): UpgradeConfig[] {
  const list: UpgradeConfig[] = [];
...
function generateWorldUpgrades(): UpgradeConfig[] {
  const list: UpgradeConfig[] = [];
...
const MARKETING_UPGRADES: UpgradeConfig[] = [
...
const RESEARCH_UPGRADES: UpgradeConfig[] = [
...
const AUTOMATION_UPGRADES: UpgradeConfig[] = [
...
const SPECIAL_UPGRADES: UpgradeConfig[] = [
...
export const UPGRADE_DEFS: UpgradeConfig[] = [
  ...CLICK_UPGRADES,
  ...AUTOMATION_UPGRADES,
  ...MARKETING_UPGRADES,
  ...RESEARCH_UPGRADES,
...
export const UPGRADE_CATEGORIES: Record<UpgradeCategory, string> = {
  click: '🖱️ Klick',
