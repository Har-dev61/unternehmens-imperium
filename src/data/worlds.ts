/**
 * World definitions — the heart of the game's content.
 *
 * Every world reuses one balanced "asset ladder" (the classic idle-game cost
 * curve) scaled by a single per-world factor applied to BOTH cost and output.
 * Keeping cost-scale === production-scale means the payback time of each tier
 * is identical in every world, so the economy stays balanced while the raw
 * numbers escalate dramatically (×10^18 from the first world to the last).
 *
 * Worlds unlock off lifetime earnings, so prestige never re-locks them.
 */
import type { AssetConfig, AssetType, WorldConfig } from '../types.js';
import type { Game } from '../core/Game.js';

interface AssetDef { name: string; icon: string; type: AssetType; desc: string; }

const LADDER: { cost: number; prod: number }[] = [
  { cost: 15, prod: 0.1 },
  { cost: 100, prod: 1 },
  { cost: 1_100, prod: 8 },
  { cost: 12_000, prod: 47 },
