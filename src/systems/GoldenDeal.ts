...
export type GoldenRewardType = 'lucky' | 'frenzy' | 'clickFrenzy';

export interface GoldenActive {
  id: string;
  type: GoldenRewardType;
  expiresAt: number;
}

const REWARD_WEIGHTS: { type: GoldenRewardType; weight: number }[] = [
...
export class GoldenDeal {
  ...
  update(dt: number): void {
    ...
    if (this.secondsUntilNext <= 0) this.spawn();
  }

  /** Consume the active deal if the id matches; returns it (or null). */
  claim(id: string): GoldenActive | null {
  ...
  private rollType(): GoldenRewardType {
